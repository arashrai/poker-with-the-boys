...
static ADMIN_ADJUSTMENT_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"\"The admin updated the player \"\"(.*?) @ \S+ stack from (\d+) to (\d+)"#)
        .unwrap()
});
static BUY_IN_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r#"\"The player \"\"(.*?) @ .* joined the game with a stack of (\d+)\.\",[^,]+,(\d+)"#,
    )
    .unwrap()
});
static REBUY_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"\"The player \"\"(.*?) @ .* rebought\. New stack (\d+)\.\",[^,]+,(\d+)"#).unwrap()
});
...
static EXIT_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r#"\"The player \"\"(.*?) @ \S+ quits the game with a stack of (\d+)\.\",[^,]+,(\d+)"#,
    )
    .unwrap()
});
...
static PLAYER_WINNER_WITH_HAND_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r#"\"\"(.*?) @ \S+ collected (\d+) from pot with (.*?)""#).unwrap());
...
static PLAYER_NAME_REGEX: Lazy<Regex> = Lazy::new(|| Regex::new(r#"[\"]{2,3}(\S+) @ "#).unwrap());
static PLAYER_INITIAL_POST_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r#"\"\"(.*?) @ \S+\"\" posts a (straddle|big blind|small blind) of (\d+)\",[^,]+,(\d+)"#,
    )
    .unwrap()
});
...
static PLAYER_RAISES_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"\"\"(.*?) @ \S+\"\" raises to (\d+)(?: and go all in)?\",[^,]+,(\d+)"#).unwrap()
});
...
impl PokerRound {
    fn new(round_logs: &[String]) -> Result<Self> {
        ...
        for row in round_logs {
            ...
            } else if let Some(caps) = REBUY_REGEX.captures(row) {
                let player = clean_player_name(&caps[1]);
                let rebuy = PlayerMovement {
                    amount: caps[2].parse()?,
                    time: from_unix_time(&caps[3])?,
                    is_rebuy: true,
                };
                player_game_joins.entry(player).or_default().push(rebuy);
        ...
        for poker_round in &self.rounds {
            for (player, joins_array) in &poker_round.player_game_joins {
                for join in joins_array {
                    if join.time < poker_round.start_time {
                        let mut buyin =
                            player_buyin_amount.get(player).copied().unwrap_or(0) + join.amount;
            ...
            for (player, balance) in &poker_round.player_balances {
                let adjusted_balance =
                    *balance - player_adjustments.get(player).copied().unwrap_or(0);
                let buyin = player_buyin_amount.get(player).copied().unwrap_or(0);
...
fn graph_stack_history(
    ...
    let mut file_name = if show_event_points {
        format!(
            "{}_all_time_profit_graph.png",
            last_file.trim_end_matches(".csv")
        )
    } else {
...
const SPLITWISE_API_URL: &str = "https://secure.splitwise.com/api/v3.0";

fn splitwise_email_for_player(player: &str) -> Option<&'static str> {
    match player {
        "Arash" => Some("arashrai17@gmail.com"),
        "Prilik" => Some("danielprilik@gmail.com"),
        "George" => Some("georgeutsin@gmail.com"),
        "Jonah" => Some("jonahdlin@gmail.com"),
        "Spencer" => Some("dobrik.spencera@gmail.com"),
        "Stephen" => Some("stephen.melinyshyn@gmail.com"),
        _ => None,
    }
}

fn splitwise_request(
    client: &Client,
    method: Method,
    path: &str,
    token: &str,
    ...
    let mut request = client
        .request(method, format!("{SPLITWISE_API_URL}{path}"))
        .bearer_auth(token)
        .header("Accept", "application/json")
        .header("User-Agent", "poker-with-the-boys/1.0");
    if let Some(form) = form {
        request = request.form(form);
    }

    let response = request.send().context("requesting Splitwise API")?;
    let status = response.status();
    let body = response.text().context("reading Splitwise API response")?;
    ...
    if !status.is_success() {
        ...
        bail!(
            "Splitwise API request failed ({}{}) . Verify that SPLITWISE_API_TOKEN is the personal API key from the app details page.",
            status,
            detail
        );
    }
    if let Some(errors) = payload.get("errors").filter(|errors| !errors.is_null()) {
        bail!("Splitwise API error: {errors}");
    }
    Ok(payload)
}

fn choose_splitwise_group(groups: &[Value]) -> Result<Value> {
    let matching: Vec<Value> = groups
        ...
        .filter(|group| {
            group
                ...
                .unwrap_or_default()
                .starts_with("Poker Night")
        })
        .cloned()
        .collect();
    if matching.is_empty() {
        bail!("Could not find a Splitwise group whose name starts with \"Poker Night\"");
    }

    println!("Available Splitwise groups:");
    for (index, group) in matching.iter().enumerate() {
        println!(
            "{}. {} (id {})",
            index + 1,
            group
                ...
                .and_then(Value::as_str)
                .unwrap_or("(unnamed)"),
            group.get("id").and_then(Value::as_i64).unwrap_or_default()
        );
    }
    ...
    loop {
        ...
        if io::stdin().read_line(&mut selection)? == 0 {
            bail!("No Splitwise group was selected");
        }
        let Some(index) = selection
            ...
            println!("Please enter a number from 1 to {}.", matching.len());
            continue;
        };
        if let Some(group) = matching.get(index) {
...
fn map_splitwise_members(
    players: &IndexMap<String, i64>,
    members: &[Value],
) -> Result<HashMap<String, Value>> {
    let mut members_by_email = HashMap::new();
    for member in members {
        if let Some(email) = member.get("email").and_then(Value::as_str) {
            if !email.is_empty() {
                members_by_email.insert(email.to_lowercase(), member.clone());
            }
        }
    }
    if members_by_email.is_empty() {
        bail!("The selected Splitwise group has no members with email addresses");
    }

    let has_unknown_player = players
        .keys()
        .any(|player| splitwise_email_for_player(player).is_none());
    if has_unknown_player {
        println!("Users in the selected Splitwise group:");
        for member in members_by_email.values() {
            println!(
                "- {} (id {})",
                member.get("email").and_then(Value::as_str).unwrap_or(""),
                member.get("id").and_then(Value::as_i64).unwrap_or_default()
            );
        }
    }
    ...
    for player in players.keys() {
        let email = if let Some(email) = splitwise_email_for_player(player) {
            email.to_string()
        } else {
            print!("Splitwise email for {player}: ");
            io::stdout().flush()?;
            let mut input = String::new();
            io::stdin().read_line(&mut input)?;
            input.trim().to_lowercase()
        };
        let member = members_by_email.get(&email.to_lowercase()).ok_or_else(|| {
            anyhow!("Splitwise email {email} for {player} is not in the selected group")
        })?;
        let member_id = member
            .get("id")
            .and_then(Value::as_i64)
            .ok_or_else(|| anyhow!("Splitwise member {email} has no numeric id"))?;
        if !used_ids.insert(member_id) {
            bail!("Splitwise member {email} was selected more than once");
        }
        mapped.insert(player.clone(), member.clone());
    }
    Ok(mapped)
}

fn add_splitwise_expense(
    player_history: &IndexMap<String, Vec<(i64, DateTime<Local>)>>,
    event_date: NaiveDate,
    token: &str,
) -> Result<()> {
    let profits: IndexMap<String, i64> = player_history
        .iter()
        .filter_map(|(player, entries)| entries.last().map(|(amount, _)| (player.clone(), *amount)))
        .collect();
    let checksum: i64 = profits.values().sum();
    if checksum != 0 {
        bail!("Cannot post non-zero-sum poker night balances ({checksum} cents)");
    }
    let positive_total: i64 = profits.values().filter(|amount| **amount > 0).sum();
    if positive_total <= 0 {
        bail!("Cannot post a poker night with no positive balance");
    }

    let client = Client::builder().build()?;
    let groups_response = splitwise_request(&client, Method::GET, "/get_groups", token, None)?;
    let groups = groups_response
        .get("groups")
        .and_then(Value::as_array)
        .ok_or_else(|| anyhow!("Splitwise response did not contain groups"))?;
    let group = choose_splitwise_group(groups)?;
    let group_id = group
        .get("id")
        .and_then(Value::as_i64)
        .ok_or_else(|| anyhow!("selected Splitwise group has no numeric id"))?;
    let members = group
        .get("members")
        .and_then(Value::as_array)
        .cloned()
    ...
    let description = format!("Poker Night {event_date}");
    let existing = splitwise_request(
        &client,
        Method::GET,
        &format!("/get_expenses?group_id={group_id}"),
        token,
        None,
    )?;
    let existing_expenses = existing
        .get("expenses")
        .and_then(Value::as_array)
        .map(Vec::as_slice)
        .unwrap_or(&[]);
    if existing_expenses.iter().any(|expense| {
        expense.get("description").and_then(Value::as_str) == Some(&description)
            && expense
                .get("date")
                .and_then(Value::as_str)
                .unwrap_or_default()
                .starts_with(&event_date.to_string())
    }) {
        eprintln!("Warning: Splitwise expense \"{description}\" already exists; skipping creation");
        return Ok(());
    }

    let mapped_members = map_splitwise_members(&profits, &members)?;
    let mut form = HashMap::from([
        (
            "cost".to_string(),
            format!("{:.2}", positive_total as f64 / 100.0),
        ),
        ("description".to_string(), description),
        (
            "details".to_string(),
            "Generated by the poker-with-the-boys script".to_string(),
        ),
        ("date".to_string(), event_date.to_string()),
        ("group_id".to_string(), group_id.to_string()),
        ("currency_code".to_string(), "CAD".to_string()),
    ]);
    for (index, (player, amount)) in profits.iter().enumerate() {
        let user_id = mapped_members[player]
            .get("id")
            .and_then(Value::as_i64)
            .ok_or_else(|| anyhow!("mapped Splitwise member for {player} has no id"))?;
        form.insert(format!("users__{index}__user_id"), user_id.to_string());
        form.insert(
            format!("users__{index}__paid_share"),
            format!("{:.2}", (*amount).max(0) as f64 / 100.0),
        );
        form.insert(
            format!("users__{index}__owed_share"),
            format!("{:.2}", (-*amount).max(0) as f64 / 100.0),
        );
    }

    let response = splitwise_request(&client, Method::POST, "/create_expense", token, Some(&form))?;
    let expense_id = response
        .get("expenses")
        .and_then(Value::as_array)
        .and_then(|expenses| expenses.first())
        .and_then(|expense| expense.get("id"))
        .and_then(Value::as_i64)
        .map(|id| id.to_string())
        .unwrap_or_else(|| "(unknown id)".to_string());
    println!("Added Splitwise expense {expense_id} to group {group_id} for {event_date}");
    Ok(())
}

fn poker_round_for_timestamp<'a>(
    rounds: &'a [PokerRound],
    time: DateTime<Local>,
) -> Option<&'a PokerRound> {
    rounds
...
fn most_wins(
    rounds: &[PokerRound],
) -> Option<(String, Vec<PokerRound>, HashMap<String, Vec<PokerRound>>)> {
...
fn largest_raise_or_bet_for_round_actions(
    round_actions: &[PlayerRoundAction],
) -> Option<PlayerRoundAction> {
    round_actions
        .iter()
...
fn all_ins_per_player(
    round_actions: &[PlayerRoundAction],
) -> IndexMap<String, Vec<PlayerRoundAction>> {
    let mut all_ins: IndexMap<String, Vec<PlayerRoundAction>> = IndexMap::new();
    for action in round_actions {
        if action.all_in {
            all_ins
                .entry(action.player.clone())
                .or_default()
                .push(action.clone());
        }
    }
...
fn winning_hand_types_by_player(
    rounds: &[PokerRound],
) -> (HashMap<String, HashMap<String, usize>>, usize) {
    ...
    for player in player_order {
        ...
        sorted.sort_by(|(h1, _), (h2, _)| {
            let r1 = hand_display_sort_order
                .get(h1.trim())
                .copied()
                .unwrap_or(999);
            let r2 = hand_display_sort_order
                .get(h2.trim())
                .copied()
                .unwrap_or(999);
            r1.cmp(&r2)
...
fn print_all_in_stats(
    ...
    for (player, all_ins) in data {
        ...
        println!(
            "{player: >10} {: >10} {all_in_pct:.2}% (Won {win_pct:.2}%)",
            all_ins.len()
        );
    }
}
...
fn print_core_stats(rounds: &[PokerRound]) {
    ...
    let mut all_player_wins_list: Vec<(String, Vec<PokerRound>)> =
        all_player_wins.drain().collect();
    ...
    let mut gent_scores_by_player: Vec<(String, usize)> =
        gentleman_scores_by_player(rounds).into_iter().collect();
    ...
    let mut player_pre_flop_folds: Vec<(String, usize)> =
        number_of_folds_per_player(&all_pre_flop_actions)
            .into_iter()
            .collect();
    let mut player_pre_turn_folds: Vec<(String, usize)> =
        number_of_folds_per_player(&all_pre_turn_actions)
            .into_iter()
            .collect();
    let mut player_pre_river_folds: Vec<(String, usize)> =
        number_of_folds_per_player(&all_pre_river_actions)
            .into_iter()
            .collect();
    let mut player_post_river_folds: Vec<(String, usize)> =
        number_of_folds_per_player(&all_post_river_actions)
            .into_iter()
            .collect();
    ...
    let wins_for_pre_flop =
        player_wins_for_round_actions(rounds, &all_ins_per_player(&all_pre_flop_actions));
    let wins_for_pre_turn =
        player_wins_for_round_actions(rounds, &all_ins_per_player(&all_pre_turn_actions));
    let wins_for_pre_river =
        player_wins_for_round_actions(rounds, &all_ins_per_player(&all_pre_river_actions));
    let wins_for_post_river =
        player_wins_for_round_actions(rounds, &all_ins_per_player(&all_post_river_actions));
...
fn run() -> Result<()> {
    ...
    if args.splitwise && args.all {
        bail!("--splitwise cannot be used with --all; choose one game-night date");
    }
    if args.splitwise && std::env::var_os("SPLITWISE_API_TOKEN").is_none() {
        bail!("--splitwise requires a Splitwise API key. Obtain one at https://secure.splitwise.com/apps/new, then run `export SPLITWISE_API_TOKEN=<your-api-key>` and try again.");
    }
    ...
    if args.all {
        ...
        let last_csv = csv_files
            .last()
            .ok_or_else(|| anyhow!("no csv files in logs/"))?;
        ...
        if args.splitwise {
            add_splitwise_expense(
                &player_history,
                date_of_csv(&csv_file)?,
                &std::env::var("SPLITWISE_API_TOKEN")?,
            )?;
        } else {
...
mod tests {
    ...
    #[test]
    fn rebuy_is_included_in_20260715_balances() {
        let logs = read_and_prepare_logs("logs/poker_night_20260715.csv").unwrap();
        let event = PokerNightEvent::new(logs).unwrap();
        let history = event.player_stack_history();

        assert_eq!(history["Prilik"].last().unwrap().0, -1600);
        assert_eq!(
            history
                .values()
                .map(|entries| entries.last().unwrap().0)
                .sum::<i64>(),
            0
        );
    }
    ...
    #[test]
    fn splitwise_email_mapping_includes_spencer() {
        assert_eq!(
            splitwise_email_for_player("Spencer"),
